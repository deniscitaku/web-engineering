<?php

include_once "../entity/News.php";
include_once "../boundary/CrudOperations.php";

class NewsDao extends CrudOperations
{

    private static $findAllStatement =
        "SELECT * FROM news";

    private static $insertStatement =
        "INSERT INTO news(title, content, image) VALUES (:title, :content, :image)";

    private static $updateStatement =
        "UPDATE news SET title = :title, content = :content, image = :image WHERE id = :id";

    private static $deleteByIdStatement =
        "DELETE FROM news WHERE id = :id";

    public function findAll()
    {
        return $this->executeStatement(self::$insertStatement)->fetchAll(PDO::FETCH_CLASS, "News");
    }

    public function insert($data)
    {
        $this->executeStatement(self::$insertStatement, [':title' => $data->getTitle(), ':content' => $data->getContent(), ':image' => $data->getImage()]);
        return true;
    }

    public function update($data, $id)
    {
        return $this->executeStatement(self::$updateStatement,
            [':title' => $data->getTitle(), ':content' => $data->getContent(), ':image' => $data->getImage(), ':id'=>$id]);
    }

    public function delete($id)
    {
        return $this->executeStatement(self::$deleteByIdStatement, [':id'=>$id]);
    }
}